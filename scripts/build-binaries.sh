#!/usr/bin/env bash
#
# Build iropi binaries for all platforms locally.
...
#     iropi-darwin-arm64.tar.gz
#     iropi-darwin-x64.tar.gz
#     iropi-linux-x64.tar.gz
#     iropi-linux-arm64.tar.gz
#     iropi-windows-x64.zip
#     iropi-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/iropi.exe"
    else
        bun build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/iropi"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating iropi-$platform.zip..."
        (cd "$platform" && zip -r ../iropi-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating iropi-$platform.tar.gz..."
        mv "$platform" iropi && tar -czf iropi-$platform.tar.gz iropi && mv iropi "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../iropi-$platform.zip)
    else
        tar -xzf iropi-$platform.tar.gz && mv iropi "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/iropi.exe"
    else
        echo "  $OUTPUT_DIR/$platform/iropi"
    fi
done
