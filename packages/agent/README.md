# @iroennys/iropi-agent-core

Stateful agent with tool execution and event streaming. Built on `@iroennys/iropi-ai`.

## Installation

```bash
npm install @iroennys/iropi-agent-core
```
...
```typescript
declare module "@iroennys/iropi-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@iroennys/iropi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@iroennys/iropi-agent-core";
