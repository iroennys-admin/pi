...
const { skills: sourcedSkills } = await loadSourcedSkills<Source, SourcedSkill>(
	env,
	[
		source("project", join(env.cwd, ".iropi/skills")),
		source("user", join(homedir(), ".iropi/agent/skills")),
		source("path", join(env.cwd, "../../../iropi-skills")),
	],
	(skill, source) => ({ ...skill, source }),
);
const { promptTemplates: sourcedPromptTemplates } = await loadSourcedPromptTemplates<Source, SourcedPromptTemplate>(
	env,
	[source("project", join(env.cwd, ".iropi/prompts")), source("user", join(homedir(), ".iropi/agent/prompts"))],
