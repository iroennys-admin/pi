...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@iroennys/iropi-coding-agent";

export default function (iropi: ExtensionAPI) {
	iropi.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
	...
	iropi.on("session_before_fork", async (event, ctx) => {
		if (!ctx.hasUI) return;
