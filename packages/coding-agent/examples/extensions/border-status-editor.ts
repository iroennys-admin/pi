...
import type { Component, EditorTheme, TUI } from "@iroennys/iropi-tui";
import { truncateToWidth, visibleWidth } from "@iroennys/iropi-tui";
...
export default function (iropi: ExtensionAPI) {
	...
	iropi.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await iropi.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
