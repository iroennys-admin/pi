 ...
 *
 *   mkdir -p ~/.iropi/agent/extensions
 *   cp packages/coding-agent/examples/extensions/project-trust.ts ~/.iropi/agent/extensions/
 *
 * Or:
 *
 *   iropi -e packages/coding-agent/examples/extensions/project-trust.ts
 *
 * Try it in a project containing .iropi, AGENTS.md/CLAUDE.md, or .agents/skills.
 */

import type { ExtensionAPI, ProjectTrustEventResult } from "@iroennys/iropi-coding-agent";
...
export default function (iropi: ExtensionAPI) {
	...
	iropi.on("project_trust", async (event, ctx): Promise<ProjectTrustEventResult> => {
