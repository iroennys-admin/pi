...
export default function (iropi: ExtensionAPI) {
	iropi.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await iropi.exec("git", ["status", "--porcelain"]);
		...
		// Stage and commit
		await iropi.exec("git", ["add", "-A"]);
		const { code: commitCode } = await iropi.exec("git", ["commit", "-m", commitMessage]);
