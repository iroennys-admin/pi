 ...
 *
 * Runs iroiropi's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   iropi -e /path/to/pi/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
 *   - Node.js >= 23.6.0 for @iroennys-admin/gondolin
...
import { RealFSProvider, VM } from "@iroennys-admin/gondolin";
...
export default function (iropi: ExtensionAPI) {
	...
	iropi.registerCommand("gondolin", {
	...
	iropi.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
