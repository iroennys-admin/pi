...
type SkillDiscoveryMode = "iropi" | "agents";
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "iropi" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function readIropiManifestFile(packageJsonPath: string): IropiManifest | null {
	try {
		const content = readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content) as { iropi?: IropiManifest };
		return pkg.iropi ?? null;
	} catch {
		return null;
	}
}
...
function resolveExtensionEntries(dir: string): string[] | null {
	...
	if (existsSync(packageJsonPath)) {
		const manifest = readIropiManifestFile(packageJsonPath);
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "iropi");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Extension packages run inside iropi and resolve iropi APIs through loader aliases/virtual modules.
		// Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
		// equivalent bun/pnpm settings) so package managers do not install or solve host-provided
		// @iroennys/iropi-* peers. Stale auto-installed iropi peers can otherwise block updates.
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "iropi-extensions", private: true };
	...
	private collectPackageResources(
		...
		const manifest = this.readIropiManifest(packageRoot);
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof IropiManifest];
	...
	private readIropiManifest(packageRoot: string): IropiManifest | null {
	...
	private addAutoDiscoveredResources(
		...
		if (projectTrusted) {
			...
			// Project skills from .iropi/
			addResources(
				"skills",
				collectAutoSkillEntries(projectDirs.skills, "iropi"),
				projectMetadata,
		...
		// User extensions from ~/.iropi/agent/
		addResources(
			"extensions",
		...
		// User skills from ~/.iropi/agent/
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "iropi"),
			userMetadata,
