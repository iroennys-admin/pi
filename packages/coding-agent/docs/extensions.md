...
> **Placement for /reload:** Put extensions in `~/.iropi/agent/extensions/` (global) or `.iropi/extensions/` (project-local) for auto-discovery. Use `pi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
Create `~/.iropi/agent/extensions/my-extension.ts`:
...
Extensions are auto-discovered from trusted locations. Project-local `.iropi/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.iropi/agent/extensions/*.ts` | Global (all projects) |
| `~/.iropi/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.iropi/extensions/*.ts` | Project-local |
| `.iropi/extensions/*/index.ts` | Project-local (subdirectory) |
...
| Package | Purpose |
|---------|---------|
| `@iroennys/iropi-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@iroennys/iropi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@iroennys/iropi-tui` | TUI components for custom rendering |
...
Fired before pi decides whether to trust a project with dynamic configs (`.iropi` or `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.
...
```typescript
import { isToolCallEventType } from "@iroennys/iropi-coding-agent";
...
```typescript
import { isBashToolResult } from "@iroennys/iropi-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@iroennys/iropi-coding-agent";
...
```typescript
import { SessionManager } from "@iroennys/iropi-coding-agent";
...
```typescript
import type { AutocompleteItem } from "@iroennys/iropi-tui";
...
```typescript
import { withFileMutationQueue } from "@iroennys/iropi-coding-agent";
...
**Important:** Use `StringEnum` from `@iroennys/iropi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
- [read.ts](https://github.com/iroennys-admin/iropi-mono/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/iroennys-admin/iropi-mono/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/iroennys-admin/iropi-mono/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/iroennys-admin/iropi-mono/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/iroennys-admin/iropi-mono/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/iroennys-admin/iropi-mono/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/iroennys-admin/iropi-mono/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@iroennys/iropi-coding-agent";
...
} from "@iroennys/iropi-coding-agent";
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/iroennys-admin/iropi-mono/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@iroennys/iropi-coding-agent";
...
```typescript
import { Text, Component } from "@iroennys/iropi-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@iroennys/iropi-coding-agent";
import { matchesKey } from "@iroennys/iropi-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@iroennys/iropi-coding-agent";
