...
- `.iropi/` in the current directory
- `.agents/skills` in the current directory or an ancestor directory

When an interactive session starts in a project with configs in `.iropi` or `.agents/skills` and no saved decision for the current directory or a parent directory, pi follows `defaultProjectTrust` from global settings. The default value is `"ask"`, which asks whether to trust the project when UI is available. Saved decisions are stored by canonical directory in `~/.iropi/agent/trust.json`, and the closest saved decision on the current or parent path applies before the global default.

Trusting a project allows pi to load trust-gated project inputs, including:

- `.iropi/settings.json`
- `.iropi` resources such as extensions, skills, prompt templates, themes, and system prompt files
...
- avoid mounting host `~/.iropi/agent` unless the container should access host sessions, settings, and credentials
...
To report a security issue, follow the repository [Security Policy](https://github.com/iroennys-admin/iropi-mono/blob/main/SECURITY.md). Do not open a public issue for security-sensitive reports.
