...
Sessions are saved automatically to `~/.iropi/agent/sessions/`, organized by working directory.
...
- `.iropi/SYSTEM.md` for a project
- `~/.iropi/agent/SYSTEM.md` globally
...
|----------|-------------|
| `IROIROPI_CODING_AGENT_DIR` | Override config directory; default is `~/.iropi/agent` |
| `IROIROPI_CODING_AGENT_SESSION_DIR` | Override session storage directory; overridden by `--session-dir` |
| `IROIROPI_PACKAGE_DIR` | Override package directory, useful for Nix/Guix store paths |
...
| `IROIROPI_CACHE_RETENTION` | Set to `long` for extended prompt cache where supported |
