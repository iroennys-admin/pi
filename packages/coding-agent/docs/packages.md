...
By default, `install` and `remove` write to user settings (`~/.iropi/agent/settings.json`). Use `-l` to write to project settings (`.iropi/settings.json`) instead. Project settings can be shared with your team, and pi installs any missing packages automatically on startup after the project is trusted.
...
- User installs go under `~/.iropi/agent/npm/`.
- Project installs go under `.iropi/npm/`.
...
- Cloned to `~/.iropi/agent/git/<host>/<path>` (global) or `.iropi/git/<host>/<path>` (project).
...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@iroennys/iropi-ai`, `@iroennys/iropi-agent-core`, `@iroennys/iropi-coding-agent`, `@iroennys/iropi-tui`, `typebox`.
...
Use `pi config` to enable or disable extensions, skills, prompt templates, and themes from installed packages and local directories. Works for both global (`~/.iropi/agent`) and project (`.iropi/`) scopes.
