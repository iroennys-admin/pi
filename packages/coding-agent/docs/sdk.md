...
```bash
npm install @iroennys/iropi-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@iroennys/iropi-coding-agent";
...
The `Agent` class (from `@iroennys/iropi-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.iropi/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.iropi/extensions/`)
- Project skills:
  - `.iropi/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.iropi/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.iropi/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { getModel } from "@iroennys/iropi-ai";
import { AuthStorage, ModelRegistry } from "@iroennys/iropi-coding-agent";
...
// Default: uses ~/.iropi/agent/auth.json and ~/.iropi/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@iroennys/iropi-coding-agent";
...
```typescript
import { createAgentSession } from "@iroennys/iropi-coding-agent";
...
import { createAgentSession, defineTool } from "@iroennys/iropi-coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.iropi/agent/extensions/`, `.iropi/extensions/`, and settings.json extension sources.
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@iroennys/iropi-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@iroennys/iropi-coding-agent";
...
1. Global: `~/.iropi/agent/settings.json`
2. Project: `<cwd>/.iropi/settings.json`
