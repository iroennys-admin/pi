...
describe("AgentSessionRuntime session lifecycle events", () => {
	...
	async function createRuntimeHost(extensionFactory: ExtensionFactory) {
		const tempDir = join(tmpdir(), `iropi-runtime-events-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("emits session_before_switch and session_start for new and resume flows", async () => {
		...
		const { runtimeHost } = await createRuntimeHost((iropi) => {
			iropi.on("session_before_switch", (event) => {
				events.push(event);
			});
			iropi.on("session_shutdown", (event) => {
				events.push(event);
			});
			iropi.on("session_start", (event) => {
				events.push(event);
			});
		});
	...
	it("emits session_before_fork and session_start and honors cancellation", async () => {
		...
		const { runtimeHost } = await createRuntimeHost((iropi) => {
			iropi.on("session_before_fork", (event) => {
				events.push(event);
				if (cancelNextFork) {
