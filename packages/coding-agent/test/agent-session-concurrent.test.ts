...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `iropi-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(iropi) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = iropi;
			},
			(iropi) => {
		...
		expect(iropi).toBeDefined();

		iropi!.sendUserMessage("Steer from extension", { deliverAs: "steer" });
