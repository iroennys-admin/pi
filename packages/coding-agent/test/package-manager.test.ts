...
Content`,
		...
		it("should auto-discover root markdown skills from .iropi skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .iropi", async () => {
			const extDir = join(tempDir, ".iropi", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".iropi"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".iropi", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".iropi", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".iropi", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".iropi", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".iropi", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json iropi.extensions in extensions setting", async () => {
			// Create a package with iropi.extensions in package.json
			...
			// Should find the extensions declared in package.json iropi.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .iropi/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-iropi", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-iropi"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-iropi\ndescription: user iropi\n---\n");
		...
		it("should use the project .iropi dir as baseDir for project .iropi skills", async () => {
			const projectBaseDir = join(tempDir, ".iropi");
			const skillPath = join(projectBaseDir, "skills", "project-iropi", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-iropi"), { recursive: true });
			writeFileSync(skillPath, "---\nname: project-iropi\ndescription: project iropi\n---\n");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".iropi", "agent");
		...
		it("should dedupe user skill entries when ~/.iropi/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .iropi auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".iropi\n");

			const skillDir = join(tempDir, ".iropi", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with iropi manifest", async () => {
		...
		it("should keep iropi manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.iropi\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".iropi", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the iropi-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("temporary install paths", () => {
		it("should place temporary npm packages under the agent temp extension folder", () => {
			...
			expect(installPath.startsWith(join(tmpdir(), "iropi-extensions"))).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .iropi settings base", () => {
			...
			const rel = relative(join(tempDir, ".iropi"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in iropi manifest", () => {
...
export default function(api) { airopi.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
			);
		...
		it("should respect package.json iropi.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".iropi", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".iropi", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".iropi", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".iropi", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".iropi", "npm", "node_modules", "example");
