...
Skill content here.`,
		...
		it("should ignore extra markdown files in auto-discovered skill dirs", async () => {
			const skillDir = join(agentDir, "skills", "iropi-skills", "browser-tools");
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".iropi", "prompts");
			...
			const projectSkillDir = join(cwd, ".iropi", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".iropi", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".iropi", "themes"), { recursive: true });
		...
		it("should load symlinked user and project extensions once", async () => {
			...
			writeFileSync(
				...
				`export default function(iropi) {
	iropi.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".iropi"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".iropi", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".iropi", "extensions", "shared.ts"));
		});

		it("should load user extensions before trust and reuse them after trust resolves", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".iropi", "extensions");
...
export default function(iropi) {
	...
	iropi.registerCommand("user-trust", {
		...
		handler: async () => {},
			...
			writeFileSync(
				...
				`export default function(iropi) {
	iropi.registerCommand("project-trusted", {
		...
		handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			expect(extensionsResult.extensions.map((extension) => extension.path)).toEqual([
				join(cwd, ".iropi", "extensions", "project.ts"),
				join(userExtDir, "user.ts"),
			]);
		...
		it("should keep both extensions loaded when command names collide", async () => {
			...
			writeFileSync(
				join(projectExtDir, "project.ts"),
				`export default function(iropi) {
	iropi.registerCommand("deploy", {
		description: "project deploy",
		handler: async () => {},
	});
	iropi.registerCommand("project-only", {
	...
	iropi.registerCommand("user-only", {
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.iropi", async () => {
			const piDir = join(cwd, ".iropi");
