...
import { Agent } from "@iroennys/iropi-agent-core";
import { getModel, type OAuthCredentials, type OAuthProvider } from "@iroennys/iropi-ai";
...
// ============================================================================
// OAuth API key resolution from ~/.iropi/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".iropi", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.iropi/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.iropi/agent/auth.json
 */
...
/** Path to the real iropi agent config directory */
export const IROPI_AGENT_DIR = join(homedir(), ".iropi", "agent");

/**
 * Get an AuthStorage instance backed by ~/.iropi/agent/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `iropi-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
