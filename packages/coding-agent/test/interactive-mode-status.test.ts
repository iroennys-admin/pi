...
import { type AutocompleteProvider, CombinedAutocompleteProvider } from "@iroennys/iropi-tui";
...
describe("InteractiveMode.showLoadedResources", () => {
	...
	function createExtensionFixtures(): ExtensionFixture[] {
		return [
			{
				path: "/tmp/project/.iropi/extensions/answer.ts",
				sourceInfo: createSourceInfo("/tmp/project/.iropi/extensions/answer.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.iropi/extensions",
				}),
			},
			{
				path: "/tmp/project/.iropi/extensions/local-index/index.ts",
				sourceInfo: createSourceInfo("/tmp/project/.iropi/extensions/local-index/index.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.iropi/extensions",
			...
			{
				path: "/tmp/project/.iropi/npm/node_modules/iropi-markdown-preview/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.iropi/npm/node_modules/iropi-markdown-preview/extensions/index.ts",
					{
						source: "npm:iropi-markdown-preview",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.iropi/npm/node_modules/iropi-markdown-preview",
					},
				),
			},
			{
				path: "/tmp/project/.iropi/npm/node_modules/@scope/iropi-scoped/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.iropi/npm/node_modules/@scope/iropi-scoped/extensions/index.ts",
					{
						source: "npm:@scope/iropi-scoped",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.iropi/npm/node_modules/@scope/iropi-scoped",
					},
				),
			},
			{
				path: "/tmp/project/.iropi/git/github.com/HazAT/iropi-interactive-subagents/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.iropi/git/github.com/HazAT/iropi-interactive-subagents/extensions/index.ts",
					{
						source: "git:github.com/HazAT/iropi-interactive-subagents",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.iropi/git/github.com/HazAT/iropi-interactive-subagents",
					},
				),
			},
			{
				path: "/tmp/project/.iropi/git/github.com/HazAT/iropi-interactive-subagents/extensions/subagents/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.iropi/git/github.com/HazAT/iropi-interactive-subagents/extensions/subagents/index.ts",
					{
						source: "git:github.com/HazAT/iropi-interactive-subagents",
						scope: "project",
						origin: "package",
...
"[Extensions]
  @scope/iropi-scoped, answer.ts, cli-extension.ts, HazAT/iropi-interactive-subagents, HazAT/iropi-interactive-subagents:subagents, local-index, iropi-markdown-preview, user-index"`);
	});
...
"[Extensions]
  project
    /tmp/project/.iropi/extensions/answer.ts
    /tmp/project/.iropi/extensions/local-index
    git:github.com/HazAT/iropi-interactive-subagents
      extensions
      extensions/subagents
    npm:@scope/iropi-scoped
      extensions
    npm:iropi-markdown-preview
      extensions
  user
  ...
  path
	...
	test("shows context paths relative to cwd while preserving full external paths", () => {
		const home = homedir();
		const cwd = path.join(home, "Development", "iropi-mono");
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			cwd,
			contextFiles: [
				{ path: path.join(home, ".iropi", "agent", "AGENTS.md") },
		...
		expect(output).toContain("~/.iropi/agent/AGENTS.md, AGENTS.md");
	...
	test("shows full context paths when expanded", () => {
		...
		const output = renderAll(fakeThis.chatContainer).replace(/\\/g, "/");
		expect(output).toContain("[Context]");
		expect(output).toContain("~/.iropi/agent/AGENTS.md");
		expect(output).toContain("~/Development/iropi-mono/AGENTS.md");
		expect(output).not.toContain("~/.iropi/agent/AGENTS.md, AGENTS.md");
	});
