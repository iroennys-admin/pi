...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "@iroennys/iropi-ai";
import { getApiProvider } from "@iroennys/iropi-ai";
import { getOAuthProvider } from "@iroennys/iropi-ai/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `iropi-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
