...
describe("createAgentSession provider attribution headers", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `iropi-sdk-attribution-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		expect(headers?.["HTTP-Referer"]).toBe("https://iropi.dev");
		expect(headers?.["X-OpenRouter-Title"]).toBe("iropi");
	...
	it("adds OpenCode session headers", async () => {
		...
		expect(headers?.["x-opencode-client"]).toBe("iropi");
	});
