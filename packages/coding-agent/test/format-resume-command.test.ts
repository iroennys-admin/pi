...
function createTempFile(): string {
	const dir = mkdtempSync(join(tmpdir(), "iropi-format-resume-command-"));
	tempDirs.push(dir);
...
function createSessionManager(options: {
	...
	return {
		...
		getSessionDir: () => options.sessionDir ?? "/tmp/iropi-sessions",
...
describe("formatResumeCommand", () => {
	...
	it("includes unquoted safe session dirs for non-default session dirs", () => {
		...
		const sessionManager = createSessionManager({
			...
			sessionDir: "/tmp/custom-iropi-sessions",
			usesDefaultSessionDir: false,
		});

		expect(formatResumeCommand(sessionManager)).toBe(
			`${APP_NAME} --session-dir /tmp/custom-iropi-sessions --session test-session`,
		);
	});
	...
	it("returns undefined when the session file is missing", () => {
		setStdoutIsTTY(true);
		const sessionManager = createSessionManager({ sessionFile: "/tmp/iropi-missing-session.jsonl" });
